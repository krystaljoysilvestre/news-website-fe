...
import { fetchNewsArticlesByCategory, searchArticles } from '../../actions/newsAction';
...
class NavigationBar extends Component {
  ...
  render() {
    const { isSearchBarOpen, searchKey } = this.state;
    ...
    return (
      <NB.Wrapper>
        ...
        <NB.Categories hidden={isSearchBarOpen}>
          <NB.Menu onClick={() => fetchNewsArticlesByCategory('business')}>Business</NB.Menu>
          <NB.Menu onClick={() => fetchNewsArticlesByCategory('entertainment')}>Entertainment</NB.Menu>
          <NB.Menu onClick={() => fetchNewsArticlesByCategory('health')}>Health</NB.Menu>
          <NB.Menu onClick={() => fetchNewsArticlesByCategory('science')}>Science</NB.Menu>
          <NB.Menu onClick={() => fetchNewsArticlesByCategory('sports')}>Sports</NB.Menu>
          <NB.Menu onClick={() => fetchNewsArticlesByCategory('technology')}>Technology</NB.Menu>
          <NB.Menu onClick={() => fetchNewsArticlesByCategory('general')}>General</NB.Menu>
        </NB.Categories>
        <NB.Menu onClick={() => this.toggleSearchBar(!isSearchBarOpen)}>
          <NB.SearchBar active={isSearchBarOpen}>
            {isSearchBarOpen ? (
              <span style={{ flex: '1', display: 'flex', alignItems: 'center' }}>
                <NB.SearchInput onKeyDown={e => this.search(e)} id="search" type="text" autoFocus placeholder="Keywords or a phrase to search for" />
                <span onClick={() => this.toggleSearchBar(!isSearchBarOpen)}>{closeIcon}</span>
              </span>
            ) : searchIcon}
          </NB.SearchBar>
        </NB.Menu>
